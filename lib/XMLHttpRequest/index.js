...
XMLHttpRequest.prototype.bypass = function() {
  this._bypass = true;
};

XMLHttpRequest.prototype.realSend = function() {
  var req = this;
  var xhr = new native.XMLHttpRequest();

  req.getResponseHeader = bind(xhr.getResponseHeader, xhr);
  req.getAllResponseHeaders = bind(xhr.getAllResponseHeaders, xhr);
  req.abort = bind(xhr.abort, xhr);

  // Bind all events from fake => real XHR instance
  forEach(events, function bridgeRealAndFakeEvents(name) {
    if (support.xhr.addEventListener) {
      if (req._eventEmitter.listeners(name).length > 0) {
        xhr.addEventListener(name, function() {
          cloneXhrValues(req, xhr);
          var args = [name].concat([].slice.call(arguments));
          req._eventEmitter.emit.apply(req._eventEmitter, args);
        });
      }
    }
  ...
  xhr.onreadystatechange = function() {
    ...
    readyStateChange(req, xhr.readyState);
  };

  xhr.open(req.requestMethod, req.requestURL, req.async, req.username, req.password);
...
function cloneXhrValues(dest, src) {
  forEach(valuesToCopy, function(prop) {
    dest[prop] = src[prop];
  });
  return dest;
}
